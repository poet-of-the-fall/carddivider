...
\begin{tikzpicture}
	...
	\cardprice{2}
	\cardtitle{Goldmünze}
	\cardcontent{Wenn du diese Karte ausspielst, bekommst du kein \coin, sonder legst einen Marker auf die Talerseite deines Taler-/Dorfbewohner-Tableaus und erhältst +1 Kauf.
	
	\medskip

	Wenn du diese Karte nimmst, darst du ein \emph{KUPFER} von deiner Hand entsorgen; dies ist optional. Die \emph{GOLDMÜNZE} ist trotzdem eine \emph{GELD}-Karte und in der Kaufphase auszuspielen, obwohl sie Anweisungen ähnlich einer Aktionskarte enthält. Sie hat einen \emph{GELD}-Wert von 0.}
\end{tikzpicture}
\hspace{-0.6cm}
\begin{tikzpicture}
	\card
	...
	\medskip
	\emph{Artefakt Laterne:} Wenn du die \emph{LATERNE} hast, musst du in deinen Zügen bei jedem ausgespielten Grenzposten 3 Karten vom Nachziehstapel aufdecken und 2 ablegen. In diesem Fall müssel alle 3 Karten Aktionskarten sein, damit du das \emph{HORN} erhältst. Die \emph{LATERNE} wirkt schon in dem Zug, in dem du sie bekommst.}
\end{tikzpicture}
\hspace{-0.6cm}
\begin{tikzpicture}
	\card
...
\begin{tikzpicture}
	...
	\cardprice{4}
	\cardtitle{Versteck}
	\cardcontent{Du musst eine Karte entsorgen. Flüche sind keine Punktekarten. Auch für kombinierte AKrten, von denen ein Typ \emph{PUNKTE} ist, musst du einen Fluch nehmen.}
\end{tikzpicture}
\hspace{-0.6cm}
\begin{tikzpicture}
	\card
	...
	\cardprice{5}
	\cardtitle{Alte Hexe}
	\cardcontent{Auch wenn der Fluch-Stapel leer wird oder ist und deshalb ein oder mehrere Spieler keinen Fluch nehmen können, dürfen trotzdem alle Mitspieler einen Fluch aus ihrer Hand entsorgen. Ist ein Spieler vom Effekt der \emph{ALTEN HEXE} nicht betroffen, z. B. weil er als Reaktion einen \emph{BURGGRABEN} aufgedeckt aht, nimmt er weder einen Fluch, noch darf er einen entsorgen.}
\end{tikzpicture}
\hspace{-0.6cm}
\begin{tikzpicture}
	\card
...
\begin{tikzpicture}
	...
	\cardcontent{Zuerst zihest du 3 Karten, dann prüfst du, ob dein Ablagestapel Karten enthält. Wenn dich das Ziehen der 3 KArten veranlasst hat zu mischen, ist dein Ablagestapel leer. Wenn dein Ablagestapel mindestens eine AKrte enthält, legst du 1 Marker auf die Taler-Seite deines Tableaus, und wenn denn 4 oder mehr Marker auf der Taler-Seite deines Tableaus leigen, erhältst du die \emph{SCHATZKISTE}. Du erhältst die \emph{SCHATZKISTE} nur, wenn du gerade einen Marker aufgrund der Anweisung auf dieser \emph{FREIBEUTERIN} auf die Taler-Seite deines Tableaus gelegt hast. Hast du das nicht getan, erhältst du die \emph{SCHATZKISTE} nicht, selbst wenn 4 Marker oder mehr auf der Taler-Seite deines Tableaus liegen.
...
\begin{tikzpicture}
	...
	\cardprice{5}
	\cardtitle{Gelehrter}
	\cardcontent{Wenn du durch das Ziehen der 7 Karten veranlast wirst zu mischen, ischst du die abglegten Karten mit ein.}
\end{tikzpicture}
\hspace{-0.6cm}
\begin{tikzpicture}
	\card
...
\begin{tikzpicture}
	...
	\cardcontent{Hat ein Mitspieler keine Karten auf der Hand, die \coin[2] oder merh kosten, muss er dies \enquote{beweisen}, indem er seine Handkarten aufdeckt.}
\end{tikzpicture}
\hspace{-0.6cm}
\begin{tikzpicture}
	\card
	...
	\cardprice{5}
	\cardtitle{Zepter}
	\cardcontent{Diese Karte ist eine Geldkarte und wird wie alle Geldkarten in der entsprechenden Spielphase ausgespielt. Wenn du sie ausspielst, kannst du wählen, ob sie in diesem Zug +\coin[2] bringt oder ob sie \coin[0] bringt und du dafür eine in diesem Zug ausgespielte Aktionskarte, die du noch im Spiel hast, erneut ausspielst. Dauerkarten, die in vergangenen Zügen ausgespielt wurden, können nicht noch einmal ausgespielt werden, in deisem Zug ausgespielte Dauerkarten aber schon (in diesem Fall bleibt das \emph{ZEPTER} auch so lange im Spiel, bis die Dauerkarte abgelegt wird).
...
\begin{tikzpicture}
	...
	\emph{Sternenkarte} (\coin[3]): Jedes Mal, wenn du mischst, darfst du die zu mischenden KArten durchsehen und eine auswählen, die du nach dem Mischen der restlichen KArten oben auf die gemsichten Karten legst.}
\end{tikzpicture}
\hspace{-0.6cm}
\begin{tikzpicture}
	\card
...
\begin{tikzpicture}
	...
	\linebreak
	Immer wenn du eine kombinierte Aktions-/Geldkarte ausspielst, ist sie sowohl Aktion als auc hGeld, unabhängig von der Phase, in der du dich gerade befindest. Dass du +1 Aktion in deiner Kaufphase bekommst, erlaubt dir nicht, dass du in der Phase andere Aktionskarten ausspielen darfst.}
\end{tikzpicture}
\hspace{-0.6cm}
\begin{tikzpicture}
	\card
...
\begin{tikzpicture}
	...
	\cardcontent{\emph{Horn (GRENZPOSTEN):} Solange du das \emph{HORN} hast, darfst du einmal pro eigenem Zug beim Ablegen eines \emph{GRENZPOSTENS} aus dem Spiel diesen auf deinen Nachziehstapel legen anstatt auf deinen Ablagestapel. Das \emph{HORN} wirkt schon in dem Zug, in dem du es bekommst.

	\smallskip

	\emph{Laterne (GRENZPOSTEN):} Wenn du die \emph{LATERNE} hast, musst du in deinen Zügen bei jedem ausgespielten Grenzposten 3 Karten vom Nachziehstapel aufdecken und 2 ablegen. In diesem Fall müssel alle 3 Karten Aktionskarten sein, damit du das \emph{HORN} erhältst. Die \emph{LATERNE} wirkt schon in dem Zug, in dem du sie bekommst.
