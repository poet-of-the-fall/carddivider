...
\begin{tikzpicture}
	...
	\cardprice{3}
	\cardtitle{Handelsroute}
	\cardcontent{Du erhältst + 1 Kauf und +\coin{1} pro Geldmarker, der sich zum Zeitpunkt des Ausspielens auf dem Handelsrouten-Tableau befndet. Du musst eine Handkarte entsorgen, wenn du mindestens 1 Karte auf der Hand hast.
	\\
	\medskip
	\\
...
\begin{tikzpicture}
	...
	\cardcontent{Du \emph{musst} eine Karte ziehen, \emph{darfst} 2 weitere Aktionen ausführen und in der Kaufphase einen zusätzlichen Kauf durchführen.}
\end{tikzpicture}
\hspace{-1cm}
\begin{tikzpicture}
	\card
	...
	\cardprice{4}
	\cardtitle{Bischof}
	\cardcontent{Du erhältst +\coin{1} und legst einen \victorypointtoken{1}-Marker auf dein Spieler-Tableau. Dann musst du eine Handkarte entsorgen, wenn du mindestens 1 Karte auf der Hand hast. Lege halb so viele \victorypointtoken{1}-Marker auf dein Spieler-Tableau, wie die entsorgte Karte gekostet hat. Ungerade Kosten werden abgerundet. Die \potion-Kosten (z.B. aus \emph{Alchemie}) spielen keine Rolle. So erhältst du je 2 \victorypointtoken{1}-Marker für ein entsorgtes \emph{ARBEITERDORF} (Kosten: \coin{4}), ebenso wie für ein \emph{GESINDEL} (Kosten: \coin{5}) oder einen \emph{GOLEM} (Kosten: \coin{4} und \potion). 
	\
	\medskip
	\\
	Jeder Mitspieler darf eine Handkarte entsorgen, erhält dafür aber keine Siegpunktmarker.\\}
\end{tikzpicture}
\hspace{-1cm}
\begin{tikzpicture}
	\card
	...
	\cardprice{4}
	\cardtitle{Denkmal}
	\cardcontent{Du erhältst +\coin{2} und legst einen \victorypointtoken{1}-Marker aus dem Vorrat auf dein Spieler-Tableau.}
\end{tikzpicture}
\hspace{-1cm}
\begin{tikzpicture}
	\card
...
\begin{tikzpicture}
	...
	\cardicon{banner/coin.png}
	\cardprice{5}
	\cardtitle{\scriptsize{Königliches Siegel}}
	\cardcontent{Diese Karte ist eine Geldkarte mit zusätzlichen Anweisungen. Sie hat den Wert \coin{2}.
	\\
	\medskip
	\\
...
\begin{tikzpicture}
	...
	\medskip
	\\
	Jeder Mitspieler (beginnend bei deinem linken Nachbarn) legt entweder einen \emph{FLUCH} aus seiner Hand ab oder er nimmt einen \emph{FLUCH} und ein \emph{KUPFER} vom Vorrat und legt diese ab. Dies dürfen die Spieler auch wählen, wenn der \emph{KUPFER}- oder \emph{FLUCH}-Stapel leer sind. Nimmt ein Spieler den \emph{FLUCH} und das \emph{KUPFER} und hat einen \emph{WACHTURM} auf der Hand, darf er diesen nach jeder genommenen Karte aufdecken (oder wahlweise nur nach einer) und z.B. den \emph{FLUCH} entsorgen und das \emph{KUPFER} auf den Nachziehstapel legen (oder ebenfalls entsorgen).\\}
\end{tikzpicture}
\hspace{-1cm}
\begin{tikzpicture}
	\card
	...
	\cardicon{banner/coin.png}
	\cardprice{5}
	\cardtitle{\footnotesize{Schmuggelware}}
...
\begin{tikzpicture}
	...
	\cardicon{banner/coin.png}
	\cardprice{6}
	\cardtitle{\footnotesize{Halsabschneider}}
	...
	\medskip
	\\
	Solange diese Karte im Spiel ist, legst du immer, wenn du eine Karte kaufst (nicht wenn du sie auf andere Weise nimmst), einen \victorypointtoken{1}-Marker auf dein Spieler-Tableau. Hast du zwei \emph{HALSABSCHNEIDER} im Spiel, legst du zwei \victorypointtoken{1}-Marker pro gekaufter Karte auf dein Tableau usw.\\}
\end{tikzpicture}
\hspace{-1cm}
\begin{tikzpicture}
	\card
...
\begin{tikzpicture}
	...
	\\
	Denkmal, Hort, Leihaus, Quacksalber, Schmuggelware, \textit{Bürokrat}, \textit{Dorf}, \textit{Gärten}, \textit{Kanzler}, \textit{Keller}
	\\
	}
\end{tikzpicture}
\hspace{-1cm}
\begin{tikzpicture}
	...
	\\
	Bischof, Denkmal, Halsabschneider, Hausierer, Leihaus, \textit{Anbau}, \textit{Armenviertel}, \textit{Baron}, \textit{Handlanger}, \textit{Harem}
	\\
	\smallskip
	\\
