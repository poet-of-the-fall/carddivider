...
\begin{tikzpicture}
	...
	\cardcontent{\emph{Siegestanz:}
		...
		\smallskip
		\\
		\emph{Demontage} (Intrige + \underline{Basisspiel}:)
		\\
		Bergwerk, Brücke, Geheimkammer, Kerkermeister, Trickser, Saboteur, \underline{Dieb, Spion, Thronsaal, Umbau}
		\\
		\smallskip
		\\
		\emph{Eine Hand voll} (Intrige + \underline{Basisspiel}:)
		\\
		Adlige, Burghof, Kerkermeister, Lakai, Verwalter, \underline{Bürokrat, Kanzler, Miliz, Mine, Ratsversammlung}
		\\
		\smallskip
		\\
		\emph{Untergebene} (Intrige + \underline{Basisspiel}:)
		\\
		Adlige, Baron, Lakai, Maskerade, Verwalter, Handlanger, \underline{Bibliothek, Hexe, Jahrmarkt, Keller}
		\\
	}
\end{tikzpicture}
\hspace{-1cm}
